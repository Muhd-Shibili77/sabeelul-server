...
export class HomeController {
    ...
    async classWiseLeaderBoard(req:Request,res:Response){
        try {
            ...
            res.status(StatusCode.OK).json({ success: true,message:'Fetching of classWise leaderboard is successfull', leaderboard });
    ...
    async fetchClassLeaderBoard(req:Request,res:Response){
        try {
            
            const leaderboard = await this.homeUseCase.fetchClassLeaderBoard()
