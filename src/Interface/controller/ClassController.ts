...
export class ClassController {
    ...
    async fetchClass(req:Request,res:Response){
        try {
            ...
            res.status(200).json({ success: true,message:'Fetching of classes is successfull', classes: result.classes, totalPages:result.totalPages ?? undefined });
    ...
    async addClass(req:Request,res:Response){
        try {
            ...
            res.status(200).json({ success: true,message:'Adding of class is successfull',  newClass });
    ...
    async editScore(req:Request,res:Response){
        try {
            const id: string = req.params.id;
            const {academicYear,item,score} = req.body
            const updatedClass = await this.classUseCase.editScore(id,academicYear,item,score)
            res.status(200).json({ success: true,message:'Editing of score is successfull', data: updatedClass });
        } catch (error: any) {
            console.error(error);
            res.status(500).json({ success: false, message: error.message });
        }
    ...
    async deleteScore(req:Request,res:Response){
        try {
            const id: string = req.params.id;
            const {academicYear,item} = req.body
            const updatedClass = await this.classUseCase.deleteScore(id,academicYear,item)
            res.status(200).json({ success: true,message:'Deleting of score is successfull', data: updatedClass });
        } catch (error: any) {
            console.error(error);
            res.status(500).json({ success: false, message: error.message });
        }
    ...
    async findClass(req:Request,res:Response){
        try {
            ...
            res.status(200).json({ success: true,message:'fetching of class is successfull', cls });
    ...
    async addSubject(req:Request,res:Response){
        try {
            ...
            res.status(200).json({ success: true,message:'add subject to class is successfull' });
    ...
    async deleteSubject(req:Request,res:Response){
        try {
            ...
            res.status(200).json({ success: true,message:'delete subject to class is successfull' });
