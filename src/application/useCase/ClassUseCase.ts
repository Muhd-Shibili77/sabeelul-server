...
export class ClassUseCase {
      ...
      try {
        ...
        if (fs.existsSync(oldIconPath)) {
          ...
          if (fs.existsSync(altPath)) {
            ...
            console.warn(
              `Old icon file not found at either path: ${oldIconPath} or ${altPath}`
            );
          }
        }
      } catch (error: any) {
  ...
  async addPenaltyScore(
    ...
    const updateClass = await this.classRepository.addPenaltyScore(
      id,
      academicYear,
      reason,
      penaltyScore,
  ...
  async editPenaltyScore(
    ...
    const updateClass = await this.classRepository.editPenaltyScore(
      id,
      markId,
      reason,
      penaltyScore,
  ...
  async deletePenaltyScore(classId: string, markId: string): Promise<void> {
    ...
    const updatedClass = await this.classRepository.deletePenaltyScore(
      classId,
      markId
    );
    return updatedClass;
